    ...
    if (isset($_POST['btnsave'])) {
        ...
        if (isset($_POST['txtemail'])) {
            ...
            if (mysqli_num_rows($select) > 0) {
                ...
                $insert = query("INSERT into tbl_user (username,useremail,userpassword,role,img) values('{$username}','{$useremail}','{$userpassword}','{$userrole}','{$image}')");
                confirm($insert);
                if ($insert) {
    ...
    if (isset($_POST['btnupdate'])) {
        ...
        if (isset($_POST['txtemail'])) {
            ...
            if (mysqli_num_rows($select) < 0) {
                ...
                $insert = query("UPDATE tbl_user set username='$username' , useremail='$useremail', useremail='$useremail', userpassword='$userpassword', img='$image' , role='$userrole' where user_id='$id'");
                confirm($insert);
                if ($insert) {
            ...
            Swal.fire({
              ...
              title: "Max file should be 1MB"
                ...
                if (move_uploaded_file($f_tmp, $store)) {
                    ...
                    $insert = query("INSERT into tbl_product ( product,category_id,description, saleprice,image) 
                        values('{$product}','{$category}','{$description}','{$saleprice}','{$productimage}')");
                    confirm($insert);
...
{
    $select = query("SELECT * from tbl_product order by pid ASC");
    confirm($select);
